...
class Admin extends CI_Controller
{
   ...
   /****MANAGE STUDENTS CLASSWISE*****/
    function counsel_add()
    {
        ...
        $page_data['page_name']  = 'counsel_add';
        $page_data['page_title'] = get_phrase('counsel_add');
        $this->load->view('backend/index', $page_data);
    }

	function counsel_information($class_id = '')
	{
		if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
			
		$page_data['page_name']  	= 'counsel_information';
		$page_data['page_title'] 	= get_phrase('counsel_information'). " - ".get_phrase('class')." : ".
											$this->crud_model->get_class_name($class_id);
		$page_data['class_id'] 	= $class_id;
		$this->load->view('backend/index', $page_data);
	}
    ...
    function counsel($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        if ($param1 == 'create') {
            ...
            $this->db->insert('counsel', $data);
            $counsel_id = $this->db->insert_id();
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/counsel_add/' . $data['counsel_id'], 'refresh');
        }
        if ($param2 == 'do_update') {
            $data['student_id'] = $this->input->post('student_id');
            $data['problem']    = $this->input->post('problem');
            ...
            $this->db->where('counsel_id', $param3);
            $this->db->update('counsel', $data);           
            $this->crud_model->clear_cache();
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/counsel_information/' . $param1, 'refresh');
        } 
        
        if ($param2 == 'delete') {
            $this->db->where('counsel_id', $param3);
            $this->db->delete('counsel');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/counsel_information/' . $param1, 'refresh');
        }
    }
...
//$data['name_numeric']         = $this->input->post('name_numeric');
    ...
    function section($class_id = '')
    {
        ...
        $page_data['page_title'] = get_phrase('manage_year_&_sections');
        $page_data['class_id']   = $class_id;
        $this->load->view('backend/index', $page_data);    
    }
    ...
    function class_routine($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'do_update') {
            ...
            $page_data['edit_data'] = $this->db->get_where('cousrse_routine', array(
    ...
    function manage_attendance_teacher($date='',$month='',$year='',$class_id='')
	{
		...
		{
			// Loop all the students of $class_id
            $teacher   =   $this->db->get_where('class' , array('class_id'=>$class_id))->row()->teacher_id;
            $students = $this->db->get_where('teacher' , array('teacher_id'=>$teacher))->result_array();
            foreach ($students as $row)
            {
			...
			$this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
			redirect(base_url() . 'index.php?admin/manage_attendance_teacher/'.$date.'/'.$month.'/'.$year.'/'.$class_id, 'refresh');
		}
        $page_data['date']     =	$date;
        $page_data['month']    =	$month;
        $page_data['year']     =	$year;
	...
	{
		redirect(base_url() . 'index.php?admin/manage_attendance_teacher/'.$this->input->post('date').'/'.
					$this->input->post('month').'/'.
						$this->input->post('year').'/'.
							$this->input->post('class_id') , 'refresh');
	}
    ...
    function fee_structure($param1 = '', $param2 = ''){
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url() . 'index.php?login', 'refresh');

        if($param1 == 'do_update'){
            $this->db->update('fee_structure',$_POST);
            $this->session->set_flashdata('flash_message' , 'Fee information updated');
            redirect(base_url() . 'index.php?admin/fee_structure/', 'refresh');
        }
    
        $page_data['page_name'] = 'fee_structure';
        $page_data['page_title'] = 'Fee Structure';
